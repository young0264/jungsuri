...
nohup java -jar -Dspring.profiles.active=prod -DrdsUrl=$rdsUrl -DdbPassword=$dbPassword -Dprofile=$profile -DweatherAPIKey=$weatherAPIKey -DmailSendLoginId=$mailSendLoginId -DmailSendPassword=$mailSendPassword -jooqVersion=$jooqVersion -accessKey=$accessKey -DdbLoginId=$dbLoginId "$PROJECT_ROOT"/jungsuri-0.0.1-SNAPSHOT.jar > "$APP_LOG" 2> "$ERROR_LOG" &
