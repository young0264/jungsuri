...
#이건가 #nohup java -jar -Dspring.profiles.active=prod "$PROJECT_ROOT/jungsuri-0.0.1-SNAPSHOT.jar" > "$APP_LOG" 2 > "$ERROR_LOG &" #& 위치 등 잘못
nohup java -jar -Dspring.profiles.active=prod "$PROJECT_ROOT/jungsuri-0.0.1-SNAPSHOT.jar" > "$APP_LOG" 2 > "$ERROR_LOG" & #& 위치 등 잘못
#nohup java -jar -Dspring.profiles.active=prod -DrdsUrl=$rdsUrl -DdbPassword=$dbPassword -Dprofile=$profile -DweatherAPIKey=$weatherAPIKey -DmailSendLoginId=$mailSendLoginId -DmailSendPassword=$mailSendPassword -DjooqVersion=$jooqVersion -DaccessKey=$accessKey -DdbLoginId=$dbLoginId "$PROJECT_ROOT"/jungsuri-0.0.1-SNAPSHOT.jar > "$APP_LOG" 2> "$ERROR_LOG" &
